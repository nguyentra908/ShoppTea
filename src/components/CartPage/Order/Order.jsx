...
const Order = (props) => {
  ...
  if (!items || (items && !items.length)) {
    swal("No products in the Cart!").then((value) => {
      window.scrollTo(0, 0);
  ...
  const submitForm = (e) => {
    ...
    swal("Order complete!").then((value) => {
      window.scrollTo(0, 0);
      history.push("/");
      dispatch(resetCart());
    });
    ...
    return (
      <div className="oder_page">
        <form>
          ...
          >
            {" "}
            <i className="fa fa-angle-double-left"></i> Back to Cart
          </span>
        </form>
      </div>
    );
  ...
  return (
    <div style={{ marginTop: "200px", marginBottom: 20 }} className="container">
      ...
      <div className="row">
        ...
        <div
          className="list-items col-md-5 payment-info-"
          style={{ marginTop: 16 }}
        >
          {renderListProduct}
